...
export default {
  ...
  methods: {
    ...
    async fetchWeightData() {
      ...
      // Create a heightData with age and height
      const weightDataMap = data.data.reduce((map, item, index) => {
          const lengthValue = labels[index];
          map[lengthValue] = item.length;
          return map;
      }, {});

      // Prepare the data with labels and heights
      const weightDataset = labels.map(label => {
          const lengthValue = parseFloat(label, 10);
          return {
              x: lengthValue,
              y: weightDataset[lengthValue] !== undefined ? weightDataMap[lengthValue] : 0 // Default to 0 if no match
          };
      });
