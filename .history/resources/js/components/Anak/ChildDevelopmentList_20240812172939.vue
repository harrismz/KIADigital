...
export default {
  ...
  methods: {
    ...
    async fetchWeightData() {
      ...
      // Step 1: Create a map from height to all weights
      ...
      // Step 2: Prepare the dataset with all weights for each length
      const weightDataset = data.weight.flatMap(item => {
          const length = item.length;
          const weights = heightToWeightsMap[length] || [0]; // Default to [0] if no weights
