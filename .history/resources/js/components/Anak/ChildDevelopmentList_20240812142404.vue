  ...
  methods: {
    ...
    async fetchChartData() {
      ...
      const labels = data.height.map(item => item.month);
       ...
       // Convert date_of_birth to age in months for each data point
       const dob = new Date(data.data[0].child.date_of_birth);
        const currentMonth = new Date().getMonth();
        const currentYear = new Date().getFullYear();
        const dobMonth = dob.getMonth();
        const dobYear = dob.getFullYear();
        
        const calculateAgeInMonths = (dob, now) => {
            let yearsDifference = now.getFullYear() - dob.getFullYear();
            let monthsDifference = now.getMonth() - dob.getMonth();
        ...
        // Use ageInMonths in your data processing if needed
        console.log({ data, labels, ageInMonths });
      ...
      this.chartData = {
        ...
        datasets: [
          ...
          {
            label: 'Child Height',
            data: data.data.map(item => item.height),
