...
export default {
  ...
  methods: {
    ...
    async fetchChartData() {
      ...
      const heightData = data.data.map(item => ({
          height: item.height,
          age: dob[data.data.indexOf(item)] // Ensure the index matches
      }));
      ...
      this.chartData = {
        ...
        datasets: [
          ...
          {
            label: 'Child Height',
            // Match the height data with the age data
            data: heightData.map(item => ({
                x: item.age,     // X-axis will be age
                y: item.height   // Y-axis will be height
            })),
