...
export default {
  ...
  methods: {
    ...
    async fetchWeightData() {
      ...
      const weightData = data.data.reduce((map, item) => {
          map[item.weight] = item.height;
          return map;
      }, {});
      ...
      const weightDataset = labels.map(label => {
          ...
          return {
              x: lengthValue,
              y: weightData[lengthValue] !== undefined ? weightData[lengthValue] : 0 // Default to 0 if no match
          };
      });
