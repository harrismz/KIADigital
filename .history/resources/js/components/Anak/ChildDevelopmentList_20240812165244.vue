  ...
  methods: {
    ...
    async fetchWeightData() {
      ...
      const heightDataMap = data.data.reduce((map, item, index) => {
          ...
          map[lengthValue] = item.weight;
          return map;
      }, {});

      // Prepare the data with labels and heights
      const heightDataset = labels.map(label => {
          const lengthValue = parseInt(label, 10); // Assuming labels are in months, convert to integer
          return {
              x: ageValengthValuelue,
              y: heightDataMap[lengthValue] !== undefined ? heightDataMap[lengthValue] : 0 // Default to 0 if no match
          };
      });
