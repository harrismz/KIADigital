...
export default {
  ...
  methods: {
    ...
    async fetchWeightData() {
      ...
      const heightToWeightMap = data.data.reduce((map, item) => {
        map[item.height] = item.weight;
        return map;
      }, {});

      // Step 2: Prepare the dataset by comparing lengths and using weights
      const weightDataset = data.weight.map(item => {
          const length = item.length;
          // Find the weight corresponding to the length (height)
          const weightValue = heightToWeightMap[length] !== undefined ? heightToWeightMap[length] : 0; // Default to 0 if no match
          return {
              x: parseFloat(length, 10), // Length as x-coordinate
              y: weightValue // Weight as y-coordinate
          };
      });
