    ...
    <VCarousel
      ...
      <VCarouselItem>
        <div class="chart-container">
          ...
          <div style="height: 400px;">
            <line-chart v-if="chartData" :data="weightData" :options="weightOptions" />
          </div>
        </div>
      </VCarouselItem>
...
export default {
  ...
  methods: {
    ...
    async fetchWeightData() {
      ...
      const weight = data.data.map(item => item.weight);

      // Step 1: Create a map from height to average weight
      const heightToWeights = data.data.reduce((map, item) => {
          if (!map[item.height]) {
              map[item.height] = [];
          }
      ...
      const heightToAverageWeightMap = Object.keys(heightToWeights).reduce((map, height) => {
          const weights = heightToWeights[height];
          const averageWeight = weights.reduce((sum, weight) => sum + weight, 0) / weights.length;
          map[height] = averageWeight;
          return map;
      }, {});

      // Step 2: Prepare the dataset using average weights
      const weightDataset = data.weight.map(item => {
          const length = item.length;
          const weightValue = heightToAverageWeightMap[length] !== undefined ? heightToAverageWeightMap[length] : 0;
          return {
              x: parseFloat(length, 10), // Length as x-coordinate
              y: weightValue // Average weight as y-coordinate
          };
      });
